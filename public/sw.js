const config = {
    cache: { // Note: html, js, and css caching is handled automatically
        version: 3, // bump to bust cache for all assets
        include: ['./manifest.json'], // paths to static files in /public that you want precached. add a version with ?v= to clean up old ones, the actual file name can remain the same.
...
self.addEventListener('fetch', event => {
    ...
    // 自定义 1：如果请求是图片且不是本网站的资源
    if ((event.request.destination === 'image' || event.request.destination === 'audio') && new URL(event.request.url).origin !== location.origin) {
